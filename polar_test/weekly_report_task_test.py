...
def weekly_report_task():
    ...
    print('trade_info.columns.', trade_info.columns)
    # 总市值计算
    ...
    # 全量计算
    # token_num_df = trade_info.groupby(['contract_address', 'token_id']).count()
    ...
    # 周初次交易统计计算
    before_trade_set = trade_info.filter(pl.col('block_number').cast(int) < start_block_weekly)[
    ...
    # week_tradet_set = week_trade_df['contract_address'].unique()
    ...
    # 周初次Mint统计计算
    week_mint_collection = contract_info.filter(pl.col('init_block') >= start_block_weekly).shape[0]
    print('本周初次Mint的Collection数为：', week_mint_collection)
    logger.info('\n本周初次Mint的Collection数为：',
                week_mint_collection)

    # all time NFT持仓者数统计
    # holder_df = transfer_info.groupby(['contract_address'])
    holder_df = transfer_info.groupby(['contract_address']).agg(
